# 🎮 Maze Run

A 3D maze game developed in Godot Engine during my final year at Mount Albert Grammar School.

![Godot Engine](https://img.shields.io/badge/Godot-4.0-478CBF?logo=godot-engine&logoColor=white)
![GDScript](https://img.shields.io/badge/GDScript-355570?logo=godot-engine&logoColor=white)

## 🏐 About

Maze Run is a 3D rolling ball maze game. Navigate through three progessivly challenging levels, avoid enemies and collect coins to win the game! The game features:

- **3 Unique Levels** - Progressive difficulty with custom-designed mazes
- **Physics-Based Movement** - Rolling ball mechanics with realistic physics
- **3D Theme** - Custom 3D models created in Blender with textures
- **Score System** - Collect coins to increase your score
- **Menu System** - Intuitive UI with replay options

## 🕹️ Controls

- **W / Up Arrow** - Move Forward
- **A / Left Arrow** - Move Left
- **S / Down Arrow** - Move Backward
- **D / Right Arrow** - Move Right

## 🛠️ Built With

- **Godot Engine 4.0** - Game engine
- **Blender** - 3D modeling and asset creation
- **GDScript** - Programming language

## 📦 Installation & Running

### Prerequisites
- [Godot Engine 4.0](https://godotengine.org/download) or later

### Running from Source
1. Clone this repository:
   ```bash
   git clone https://github.com/Chrish1603/Maze-Run.git
   ```

2. Open Godot Engine

3. Click "Import" and navigate to the cloned folder

4. Select `project.godot` and click "Import & Edit"

5. Press **F5** or click the Play button to run the game

### Playing the macOS Build
Download `Maze Run VS.dmg` from the repository and run it directly on macOS.

## 📁 Project Structure

```
Maze-Run/
├── assets/          # Game assets and resources
├── fonts/           # Custom fonts
├── imports/         # Imported 3D models
├── scenes/          # Game scenes
├── scripts/         # GDScript files
├── sprites/         # 2D sprites
├── textures/        # 3D model textures
├── level1.tscn      # Level 1 scene
├── level2.tscn      # Level 2 scene
├── level3.tscn      # Level 3 scene
├── menu.tscn        # Main menu
└── project.godot    # Godot project file
```

## 🎓 Development Journey

This project was developed as part of my Digital Technology subject at Mount Albert Grammar School. It represents:

- Learning 3D game development from scratch
- Understanding physics-based gameplay mechanics
- Creating custom 3D assets in Blender
- Implementing UI/UX design principles
- Version control with Git and GitHub

### Development Timeline

- **July 2022** - Initial prototype and 2D concept
- **June 2023** - Transition to 3D gameplay
- **August 2023** - Vertical slice with core mechanics
- **October 2023** - Multi-level system and final polish

## 🤝 Contributing

This is a personal educational project, but feedback and suggestions are welcome!

## 📝 License

This project is open source.

## 👨‍💻 Author

**Chrish Patel**  
Mount Albert Grammar School - Class of 2023

---

*Made with Godot Engine and Blender*
